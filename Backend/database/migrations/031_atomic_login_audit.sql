...
begin
  ...
  update public.users set last_login_at=clock_timestamp() where id=p_user_id;
  insert into public.audit_logs(user_id,action,entity_type,metadata)
    values(p_user_id,'LOGIN_SUCCESS','auth',jsonb_build_object('username',account.username,'role',(select name from public.roles where id=account.role_id)));
  return session_id;
end $$;
