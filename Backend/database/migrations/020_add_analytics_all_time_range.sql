...
-- All-time requests are still bounded to a defensive 100-year window and are
-- aggregated by calendar month by the backend.
  ...
  select
    case
      when p_bucket_seconds = 0 then
        ...
        + (
          floor(extract(epoch from (event.recorded_at - p_started_at)) / p_bucket_seconds)
          * p_bucket_seconds
        ) * interval '1 second'
    end as bucket_start,
    sensor.sensor_code,
